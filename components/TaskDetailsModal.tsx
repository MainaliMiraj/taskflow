...
const STATUS_DETAILS: { value: TaskStatus; label: string; helper: string }[] = [
  ...
  { value: 'in-progress', label: 'In Progress', helper: 'Currently being worked on' },
  { value: 'completed', label: 'Completed', helper: 'Finished and ready to archive' },
];
...
export default function TaskDetailsModal({
  ...
  return (
    ...
    >
      ...
      >
        <div className="absolute right-6 top-6 flex items-center gap-2 text-xs font-medium uppercase tracking-[0.15em] text-gray-400">
          Status
          <span className="rounded-full bg-slate-100 px-3 py-1 text-gray-600">
            {task.status.replace('-', ' ')}
          </span>
        </div>
        <button
          className="absolute right-4 top-4 rounded-full p-2 text-gray-500 transition hover:bg-slate-100 hover:text-gray-900"
          onClick={onClose}
        ...
        <div className="bg-gradient-to-br from-white via-primary-50/60 to-white px-8 pb-6 pt-10 sm:px-10">
          ...
          <div className="mt-6 grid gap-4 text-sm text-gray-600 sm:grid-cols-2">
            ...
            <div>
              ...
              <p className="font-mono text-sm text-gray-800">{task.id}</p>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-slate-100 bg-white px-8 py-6 sm:px-10">
          ...
          <div className="mt-8 flex flex-col gap-3 border-t border-slate-100 pt-6 sm:flex-row sm:justify-end">
            <button
              className="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-gray-700 transition hover:border-gray-400 sm:w-auto"
              onClick={() => onEdit(task.id)}
            >
              Edit Task
            </button>
            <button
              className="w-full rounded-xl border border-red-200 bg-red-50 px-4 py-2 text-sm font-semibold text-red-700 transition hover:bg-red-100 sm:w-auto"
